...
void constructPreset()
{
  ...
  else if (preset == presets::Construction::INCENTER)
  {
    ...
    dynamic_cast<ConstructionPoint*>(pointC->GetConstruction())->coord = exprC * exprC;
    namebox.setName("B");
    UnitPoint* pointB = new UnitPoint(UnitCircle::getInstance(), Vector2f(0, -unitSeg));
    expr exprB(make_unit_term("b"));
    dynamic_cast<ConstructionPoint*>(pointB->GetConstruction())->coord = exprB * exprB;
    namebox.setName("A");
    UnitPoint* pointA = new UnitPoint(UnitCircle::getInstance(), Vector2f(-unitSeg, unitSeg));
    expr exprA(make_unit_term("a"));
    dynamic_cast<ConstructionPoint*>(pointA->GetConstruction())->coord = exprA * exprA;
    namebox.setName("I");
    Point* pointI = new Point(pointA, pointB, pointC, Point::INCENTER);
    dynamic_cast<ConstructionPoint*>(pointI->GetConstruction())->coord = -(exprA * exprB + exprA * exprC + exprB * exprC);
    namebox.setName("Da");
    UnitPoint* pointDa = new UnitPoint(UnitCircle::getInstance(), pointI, pointA);
    dynamic_cast<ConstructionPoint*>(pointDa->GetConstruction())->coord = -exprB * exprC;
    namebox.setName("Db");
    UnitPoint* pointDb = new UnitPoint(UnitCircle::getInstance(), pointI, pointB);
    dynamic_cast<ConstructionPoint*>(pointDb->GetConstruction())->coord = -exprA * exprC;
    namebox.setName("Dc");
    UnitPoint* pointDc = new UnitPoint(UnitCircle::getInstance(), pointI, pointC);
    dynamic_cast<ConstructionPoint*>(pointDc->GetConstruction())->coord = -exprA * exprB;
    pointA->UpdateTex();
    pointB->UpdateTex();
    pointC->UpdateTex();
    pointI->UpdateTex();
...
void provingTheProblem()
{
  ...
  view.setViewport(mainWindowRect);
  moveButton.press();
  Creation::getInstance()->CurrentMethod = moveButton.GetFunction();
